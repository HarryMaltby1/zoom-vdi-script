...
Start-Process -FilePath "msiexec.exe" -ArgumentList @("/i", "ZoomAllInOnePlugin.msi", "INSTALLFORAVD=1", "/qn", "/log", "C:\Temp\ZoomPluginInstall.log") -Wait

# Install Zoom VDI with config
Start-Process msiexec.exe -ArgumentList '/i "ZoomInstallerVDI.msi" /qn ZConfig="kCmdParam_InstallOption=8;zSSOHost=plymouth.zoom.us"' -Wait
